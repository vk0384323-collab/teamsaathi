...
function openAddTrainingModal() {
  ...
  document.getElementById('trainingModalTrainerInput').value = '';
...
async function saveTrainingRecord() {
  ...
  const training_date = document.getElementById('trainingModalDateInput').value;
...
async function saveReportRecord() {
  ...
  const due_date = document.getElementById('reportModalDateInput').value;
  ...
  try {
    const res = await fetch('/api/reports', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ report_name, department_id: parseInt(department_id), frequency, due_date, custom_note, added_by: currentUser.name || 'Admin' })
    });
    const json = await res.json();
    if (json.success) {
      closeModal('reportModal');
      showToast('Department report task assigned!', 'success');
      await refreshData();
    } else {
...
async function saveMeetingRecord() {
  ...
  const meeting_date = document.getElementById('meetingModalDateInput').value;
...
function renderAdminRoles() {
  ...
  grid.innerHTML = roles.map(r => {
    ...
    return `
      <div class="card" style="border-top: 4px solid var(--lake); display: flex; flex-direction: column; justify-content: space-between;">
        <div>
          <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 12px;">
            <div style="width: 44px; height: 44px; border-radius: 12px; background: #EFF6FF; border: 1.5px solid #BFDBFE; display: flex; align-items: center; justify-content: center; font-size: 22px;">${icon}</div>
            <div>
              <div style="font-size: 12px; font-weight: 700; color: var(--lake); text-transform: uppercase; letter-spacing: 0.04em;">${esc(r.responsibility_area)}</div>
              <h4 style="font-size: 18px; margin: 0; color: var(--ink);">${esc(r.admin_name)}</h4>
            </div>
          </div>
...
function openAddAdminRoleModal() {
  ...
  document.getElementById('adminRoleModalNameInput').value = '';
...
function openEditAdminRoleModal(id) {
  ...
  document.getElementById('adminRoleModalAreaSelect').value = r.responsibility_area || 'General Administration';
...
function renderAdminRolesTable() {
  ...
  tbody.innerHTML = list.map(r => `
    <tr>
      ...
      <td style="text-align: right;">
        <button class="btn-icon-action" onclick="openEditAdminRoleModal(${r.id})">Edit</button>
        <button class="btn-icon-action delete" onclick="deleteAdminRole(${r.id})">Delete</button>
      </td>
    </tr>
  `).join('');
}
