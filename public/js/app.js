...
function switchAdminTab(subTabId, btnEl) {
  ...
  if (subTabId === 'admin-trainings' && typeof renderAdminTrainingsTable === 'function') renderAdminTrainingsTable();
  if (subTabId === 'admin-reports' && typeof renderAdminReportsTable === 'function') renderAdminReportsTable();
  if (subTabId === 'admin-meetings' && typeof renderAdminMeetingsTable === 'function') renderAdminMeetingsTable();
  if (subTabId === 'admin-roles-panel' && typeof renderAdminRolesTable === 'function') renderAdminRolesTable();
}
...
function renderLinks() {
  ...
  // Department pills row removed from general view to maintain strict department privacy.
  const deptRow = document.getElementById('linkDeptPills');
  if (deptRow) {
    deptRow.innerHTML = '';
  }

  // The general Links view strictly displays common company-wide platforms (l.department_id is null / open to all)
  let filtered = (DATA.links || []).filter(l => {
    // Strictly common company links in general view
  ...
  if (filtered.length === 0) {
    grid.innerHTML = `
      <div class="empty-state">
        <p>No company-wide links match your search.</p>
        ${isAdmin ? `<button class="btn-primary" style="margin-top: 10px;" onclick="openAddLinkModal()">+ Add Company Link</button>` : ''}
      </div>`;
    return;
  }
  ...
  grid.innerHTML = filtered.map((l, i) => {
    ...
    return `
      <a class="card" href="${esc(l.url)}" target="_blank" rel="noopener noreferrer" style="animation-delay: ${i * 0.03}s">
        <div class="card-top">
...
function updateAdminUiState() {
  ...
  const trainingActions = document.getElementById('adminTrainingActions');
  const reportActions = document.getElementById('adminReportActions');
  const meetingActions = document.getElementById('adminMeetingActions');
  const rolesActions = document.getElementById('adminRolesActions');

  if (linkActions) linkActions.style.display = 'block';
  if (announceActions) announceActions.style.display = 'block';
  if (trainingActions) trainingActions.style.display = 'block';
  if (reportActions) reportActions.style.display = 'block';
  if (meetingActions) meetingActions.style.display = 'block';
  if (rolesActions) rolesActions.style.display = 'block';

  if (isAdmin) {
    if (adminBtn) adminBtn.classList.add('logged-in');
    if (adminBtnText) adminBtnText.textContent = 'Admin Console';
    if (pollControls) pollControls.style.display = 'block';
  } else {
    if (adminBtn) adminBtn.classList.remove('logged-in');
    if (adminBtnText) adminBtnText.textContent = 'Admin Area';
...
async function renderAdminTables() {
  ...
  renderAdminTrainingsTable();
  renderAdminReportsTable();
  renderAdminMeetingsTable();
  renderAdminRolesTable();

  // Admin action buttons visibility
  const atBtn = document.getElementById('adminTrainingActions');
  if (atBtn) atBtn.style.display = isAdmin ? 'block' : 'none';
  const arBtn = document.getElementById('adminReportActions');
  if (arBtn) arBtn.style.display = isAdmin ? 'block' : 'none';
  const amBtn = document.getElementById('adminMeetingActions');
  if (amBtn) amBtn.style.display = isAdmin ? 'block' : 'none';
  const aRolesBtn = document.getElementById('adminRolesActions');
  if (aRolesBtn) aRolesBtn.style.display = isAdmin ? 'block' : 'none';
}
...
async function deleteLink(id) {
  ...
  const previousLinks = [...DATA.links];
...
async function deleteAnnouncement(id) {
  ...
  const previousAnnounce = [...DATA.announcements];
  DATA.announcements = DATA.announcements.filter(a => a.id !== id);
  renderAnnouncements();
  renderAdminAnnounceTable();
  ...
  try {
    ...
    const json = await res.json();
    if (!json.success) {
      DATA.announcements = previousAnnounce;
      renderAnnouncements();
      renderAdminAnnounceTable();
      showToast(json.error || 'Failed to delete update', 'error');
    }
  } catch (err) {
    DATA.announcements = previousAnnounce;
    renderAnnouncements();
...
async function deleteBottleneck(id) {
  ...
  const previousBottlenecks = [...DATA.bottlenecks];
  DATA.bottlenecks = DATA.bottlenecks.filter(b => b.id !== id);
  renderAdminBottlenecks();
  ...
  try {
    ...
    const json = await res.json();
    if (!json.success) {
      DATA.bottlenecks = previousBottlenecks;
      renderAdminBottlenecks();
      showToast(json.error || 'Failed to delete bottleneck', 'error');
    }
  } catch (err) {
    DATA.bottlenecks = previousBottlenecks;
    renderAdminBottlenecks();
...
async function deleteMember(id) {
  ...
  const previousMembers = [...DATA.teamMembers];
  DATA.teamMembers = DATA.teamMembers.filter(m => m.id !== id);
  if (adminPersonalRoster) {
    adminPersonalRoster = adminPersonalRoster.filter(m => m.id !== id);
  }
  ...
  try {
    ...
    DATA.teamMembers = previousMembers;
    await refreshData();
...
async function saveLinkRecord() {
  ...
  try {
    ...
    if (json.success) {
      ...
      if (json.link) {
        const dept = (DATA.departments || []).find(d => d.id === parseInt(department_id));
        const enriched = { ...json.link, department_name: dept?.name };
        if (id) {
          const idx = (DATA.links || []).findIndex(l => l.id === parseInt(id));
          if (idx !== -1) DATA.links[idx] = enriched;
        } else {
          if (!DATA.links) DATA.links = [];
          DATA.links.unshift(enriched);
        }
        renderLinks();
        renderAdminLinksTable();
      }
      ...
      showToast(json.error || 'Failed to save link', 'error');
    }
  } catch (err) {
    showToast('Network error while saving link', 'error');
  }
}
...
function renderPersonalWindow() {
  ...
  const deptLinksCount = (DATA.links || []).filter(l => l.department_id === m.department_id).length;
  const linkBadge = document.getElementById('pTabLinksBadge');
  if (linkBadge) linkBadge.textContent = personalSession.personal_links.length + deptLinksCount;
...
function renderPersonalLinksList() {
  ...
  const personalLinks = personalSession.personal_links || [];
  const deptId = personalSession.member?.department_id;
  const deptName = personalSession.member?.department_name || 'My Department';
  const deptLinks = (DATA.links || []).filter(l => l.department_id === deptId);

  if (personalLinks.length === 0 && deptLinks.length === 0) {
    listEl.innerHTML = `
      <div class="empty-state" style="padding: 30px 10px;">
        <div style="font-size: 36px; margin-bottom: 8px;">📂</div>
        <h4>No Assigned Links Yet</h4>
        <p style="font-size: 13px; color: var(--ink-muted);">Links and portals assigned to you or the ${esc(deptName)} department will appear here.</p>
      </div>
    `;
    return;
  }
  ...
  if (deptLinks.length > 0) {
    html += `
      <div style="margin-bottom: 16px;">
        <div style="font-size: 11.5px; font-weight: 700; color: var(--lake); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; display: flex; align-items: center; gap: 6px;">
          <span>🏢</span> ${esc(deptName)} Department Platforms & Links (${deptLinks.length})
        </div>
        <div style="display: flex; flex-direction: column; gap: 8px;">
          ${deptLinks.map(l => `
            <div class="personal-link-card" style="border-left: 3px solid var(--lake);">
              <div style="display: flex; align-items: center; gap: 12px;">
                <span style="font-size: 22px;">${esc(l.icon || '🔗')}</span>
                <div>
  ...
  if (personalLinks.length > 0) {
    html += `
      <div>
        <div style="font-size: 11.5px; font-weight: 700; color: var(--teal-dark); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; display: flex; align-items: center; gap: 6px;">
          <span>📑</span> Private Documents & Personal Sheets (${personalLinks.length})
        </div>
        <div style="display: flex; flex-direction: column; gap: 8px;">
          ${personalLinks.map(l => `
            <div class="personal-link-card" style="border-left: 3px solid var(--teal);">
              <div style="display: flex; align-items: center; gap: 12px;">
...
function showCopiedFeedback(btnEl, name) {
  if (btnEl) {
    ...
    if (btnEl._copiedTimer) {
      clearTimeout(btnEl._copiedTimer);
    }
    btnEl._copiedTimer = setTimeout(() => {
      btnEl.innerHTML = btnEl.dataset.origHtml || '<span>📋</span> Copy Direct URL';
      btnEl.classList.remove('copied');
      btnEl._copiedTimer = null;
    }, 2000);
  }
...
function openEditMemberCredsModal(id) {
  ...
  const directBtn = document.getElementById('adminCredsDirectLinkBtn');
  if (directBtn) {
    directBtn.innerHTML = '<span>📋</span> Copy Direct URL';
    directBtn.classList.remove('copied');
    if (directBtn._copiedTimer) {
      clearTimeout(directBtn._copiedTimer);
      directBtn._copiedTimer = null;
    }
  }
  ...
  try {
    ...
    if (json.success) {
      ...
      if (json.announcement) {
        const dept = (DATA.departments || []).find(d => d.id === parseInt(department_id));
        const enriched = { ...json.announcement, department_name: dept?.name };
        if (editId) {
          const idx = (DATA.announcements || []).findIndex(a => a.id === parseInt(editId));
          if (idx !== -1) DATA.announcements[idx] = enriched;
        } else {
          if (!DATA.announcements) DATA.announcements = [];
          DATA.announcements.unshift(enriched);
        }
        renderAnnouncements();
        renderAdminAnnounceTable();
      }
...
function openAddAdminPasswordModal() {
  ...
  const allModules = ['links', 'trainings', 'reports', 'meetings', 'announcements', 'polls', 'bottlenecks', 'roster', 'departments', 'passwords'];
...
async function saveAdminPasswordRecord() {
  ...
  if (perms.length === allModules.length) perms.push('all');
...
function populateDropdowns() {
  ...
  const selects = ['ideaModalDeptSelect', 'hurdleModalDeptSelect', 'memberModalDeptSelect', 'reportModalDeptSelect'];
  selects.forEach(id => {
  ...
  const trainingDeptSelect = document.getElementById('trainingModalDeptSelect');
  if (trainingDeptSelect) {
    trainingDeptSelect.innerHTML = `<option value="ALL">🏢 All Departments (Company-Wide)</option>` + deptOptions;
  }

  const meetingDeptSelect = document.getElementById('meetingModalDeptSelect');
  if (meetingDeptSelect) {
    meetingDeptSelect.innerHTML = `<option value="ALL">Company-Wide (All)</option>` + deptOptions;
  }

  // Reports department filter pills
  const repPillsEl = document.getElementById('reportsDeptFilterPills');
  if (repPillsEl) {
    repPillsEl.innerHTML = `<button class="pill active" onclick="filterReportsByDept('all', this)">All Departments</button>` +
      DATA.departments.map(d => `<button class="pill" onclick="filterReportsByDept(${d.id}, this)">${d.icon} ${esc(d.name)}</button>`).join('');
  }
 ...
 * 1. Training Hub (Schedule, PDF Guides & RSVP)
 * 2. Reports Board (Department Tasks & Status Tracking)
 * 3. Company Meetings & Schedules
 * 4. Admin Responsibility Panel (Who Manages What)
 * ===================================================================== */

// ── 1. TRAINING HUB ENGINE ────────────────────────────────────
let trainingFilterStatus = 'all';

function filterTrainings(status, btn) {
  trainingFilterStatus = status;
  const pills = document.querySelectorAll('#trainingFilterPills .pill');
...
function renderTrainings() {
  ...
  if (trainingFilterStatus !== 'all') {
    filtered = filtered.filter(t => t.status === trainingFilterStatus);
  }

  if (filtered.length === 0) {
    grid.innerHTML = `<div class="empty-state" style="grid-column: 1/-1;"><p>No training sessions scheduled under '${trainingFilterStatus}'.</p></div>`;
  } else {
    grid.innerHTML = filtered.map(t => {
      let deptLabel = '🏢 All Departments';
      if (t.dept_ids && t.dept_ids !== 'ALL') {
        const ids = t.dept_ids.split(',').map(n => parseInt(n));
        const matchedDepts = DATA.departments.filter(d => ids.includes(d.id));
        deptLabel = matchedDepts.map(d => `${d.icon} ${d.name}`).join(', ') || t.dept_ids;
      }

      const isUpcoming = (t.status || 'upcoming') === 'upcoming';
      const statusBadge = isUpcoming
        ? `<span class="badge" style="background: #E0F2FE; color: #0369A1;">⏳ Upcoming</span>`
        : `<span class="badge" style="background: #DCFCE7; color: #166534;">✅ Completed</span>`;

      const guideBtn = t.guide_url ? `
        <a href="${esc(t.guide_url)}" target="_blank" rel="noopener" class="btn-secondary" style="display: inline-flex; align-items: center; gap: 6px; font-size: 12px; padding: 6px 12px; text-decoration: none;">
          <span>📄</span> View PDF Guide ↗
        </a>
      ` : '';

      return `
        <div class="card" style="display: flex; flex-direction: column; justify-content: space-between; border-top: 4px solid var(--lake);">
          <div>
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px; gap: 8px;">
              <span class="badge badge-lake" style="font-size: 11px;">${t.training_type === 'Online' ? '💻 Online Session' : '🏢 In-Person Training'}</span>
              ${statusBadge}
            </div>
            <h4 style="font-size: 17px; margin-bottom: 8px; color: var(--ink); line-height: 1.3;">${esc(t.title)}</h4>
            <div style="font-size: 12.5px; color: var(--ink-muted); margin-bottom: 12px; display: flex; flex-direction: column; gap: 4px;">
              <div>📅 <b>Date:</b> ${t.training_date ? formatDate(t.training_date) : 'TBA'} ${t.training_time ? '• ' + esc(t.training_time) : ''}</div>
              <div>👤 <b>Trainer:</b> ${esc(t.trainer_name || 'Management Lead')}</div>
              <div>👥 <b>Audience:</b> ${esc(deptLabel)}</div>
            </div>
          </div>
          <div style="border-top: 1px solid var(--border); padding-top: 12px; margin-top: 12px; display: flex; justify-content: space-between; align-items: center; gap: 8px; flex-wrap: wrap;">
            ${guideBtn || '<span style="font-size: 11.5px; color: var(--ink-muted);">No PDF attached</span>'}
            <button class="btn-primary" style="font-size: 12px; padding: 6px 14px;" onclick="handleTrainingRsvp(${t.id}, 'yes')">
              ✓ RSVP / Attend
            </button>
          </div>
        </div>
  ...
  // Render Training Guides Grid
  const guidesGrid = document.getElementById('trainingGuidesGrid');
  if (guidesGrid) {
    const guides = trainings.filter(t => t.guide_url && t.guide_url.trim().length > 0);
    if (guides.length === 0) {
      guidesGrid.innerHTML = `<div class="empty-state" style="grid-column: 1/-1;"><p>No PDF training guides or SOPs linked yet. Admins can attach guides via + Schedule Training.</p></div>`;
    } else {
      guidesGrid.innerHTML = guides.map(g => `
        <div class="card" style="display: flex; align-items: center; justify-content: space-between; gap: 14px; padding: 16px 20px;">
          <div style="display: flex; align-items: center; gap: 14px;">
            <div style="width: 44px; height: 44px; border-radius: 10px; background: #EFF6FF; border: 1.5px solid #BFDBFE; display: flex; align-items: center; justify-content: center; font-size: 22px;">📚</div>
            <div>
              <div style="font-weight: 700; font-size: 15px; color: var(--ink);">${esc(g.title)}</div>
              <div style="font-size: 12px; color: var(--ink-muted);">Trainer: ${esc(g.trainer_name || 'Management')} • ${g.training_type || 'Guide'}</div>
            </div>
          </div>
          <a href="${esc(g.guide_url)}" target="_blank" rel="noopener" class="btn-primary" style="padding: 7px 14px; font-size: 12.5px; text-decoration: none;">Open PDF ↗</a>
        </div>
      `).join('');
    }
  }
...
async function handleTrainingRsvp(trainingId, rsvpStatus) {
  let memberId = (personalSession.authenticated && personalSession.member) ? personalSession.member.id : null;
  if (!memberId && currentUser.name) {
    const found = DATA.teamMembers.find(m => m.name.toLowerCase().trim() === currentUser.name.toLowerCase().trim());
    if (found) memberId = found.id;
  }

  if (!memberId) {
    showToast('Please sign in with your Personal Window to RSVP', 'info');
    openPersonalLoginModal();
    return;
  }
  ...
  try {
    const res = await fetch(`/api/trainings/${trainingId}/rsvp`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ member_id: memberId, rsvp: rsvpStatus })
    });
    const json = await res.json();
    if (json.success) {
      showToast('RSVP confirmed! You are registered for this session.', 'success');
    } else {
      showToast(json.error || 'Failed to record RSVP', 'error');
    }
  } catch (err) {
    showToast('Network error while saving RSVP', 'error');
  }
}
...
function openAddTrainingModal() {
  ...
  document.getElementById('trainingModalEditId').value = '';
  document.getElementById('trainingModalTitleInput').value = '';
  document.getElementById('trainingModalTrainerInput').value = currentUser.name || '';
  document.getElementById('trainingModalTypeSelect').value = 'Offline';
  document.getElementById('trainingModalDateInput').value = '';
  document.getElementById('trainingModalTimeInput').value = '';
  document.getElementById('trainingModalDeptSelect').value = 'ALL';
  document.getElementById('trainingModalGuideUrlInput').value = '';
  openModal('trainingModal');
}

function openEditTrainingModal(id) {
  const t = (DATA.trainings || []).find(item => item.id === id);
  if (!t) return;
  populateDropdowns();
  document.getElementById('trainingModalTitle').textContent = 'Edit Training Session';
  document.getElementById('trainingModalEditId').value = t.id;
  document.getElementById('trainingModalTitleInput').value = t.title || '';
  document.getElementById('trainingModalTrainerInput').value = t.trainer_name || '';
  document.getElementById('trainingModalTypeSelect').value = t.training_type || 'Offline';
  document.getElementById('trainingModalDateInput').value = t.training_date ? t.training_date.split('T')[0] : '';
  document.getElementById('trainingModalTimeInput').value = t.training_time || '';
  document.getElementById('trainingModalDeptSelect').value = t.dept_ids || 'ALL';
  document.getElementById('trainingModalGuideUrlInput').value = t.guide_url || '';
  openModal('trainingModal');
}

async function saveTrainingRecord() {
  const editId = document.getElementById('trainingModalEditId').value;
  const title = document.getElementById('trainingModalTitleInput').value.trim();
  const trainer_name = document.getElementById('trainingModalTrainerInput').value.trim();
  const training_type = document.getElementById('trainingModalTypeSelect').value;
  const training_date = document.getElementById('trainingModalDateInput').value || null;
  const training_time = document.getElementById('trainingModalTimeInput').value.trim();
  const dept_ids = document.getElementById('trainingModalDeptSelect').value;
  const guide_url = document.getElementById('trainingModalGuideUrlInput').value.trim();
  ...
  const endpoint = editId ? `/api/trainings/${editId}` : '/api/trainings';
  const method = editId ? 'PUT' : 'POST';
  ...
  try {
    const res = await fetch(endpoint, {
      method,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title, trainer_name, training_type, training_date, training_time, dept_ids, guide_url, created_by: currentUser.name || 'Admin' })
    });
    const json = await res.json();
    if (json.success) {
      closeModal('trainingModal');
      showToast(editId ? 'Training updated successfully!' : 'Training session scheduled!', 'success');
      if (json.training) {
        if (editId) {
          const idx = (DATA.trainings || []).findIndex(t => t.id === parseInt(editId));
          if (idx !== -1) DATA.trainings[idx] = json.training;
        } else {
          if (!DATA.trainings) DATA.trainings = [];
          DATA.trainings.unshift(json.training);
        }
        renderTrainings();
        renderAdminTrainingsTable();
      }
      ...
      showToast(json.error || 'Failed to save training', 'error');
    }
  } catch (err) {
    showToast('Network error while saving training', 'error');
  }
}
...
async function deleteTraining(id) {
  ...
  const previousTrainings = [...(DATA.trainings || [])];
  ...
  try {
    ...
    const json = await res.json();
    if (!json.success) {
      DATA.trainings = previousTrainings;
      renderTrainings();
      renderAdminTrainingsTable();
      showToast(json.error || 'Failed to delete training', 'error');
    }
  } catch (err) {
    DATA.trainings = previousTrainings;
    renderTrainings();
    renderAdminTrainingsTable();
    showToast('Network error while deleting training', 'error');
  }
}

function renderAdminTrainingsTable() {
  const tbody = document.getElementById('adminTrainingsTableBody');
  if (!tbody) return;
  const list = DATA.trainings || [];
  if (list.length === 0) {
    tbody.innerHTML = `<tr><td colspan="8" style="text-align: center; color: var(--ink-muted); padding: 24px;">No training sessions scheduled. Click "+ Schedule Training" to add.</td></tr>`;
    return;
  }
  tbody.innerHTML = list.map(t => `
    <tr>
      <td><span class="badge ${t.status === 'completed' ? 'badge-green' : 'badge-lake'}">${esc(t.status || 'upcoming')}</span></td>
      <td><b>${esc(t.title)}</b></td>
      <td>${esc(t.trainer_name || '—')}</td>
      <td><span class="badge">${esc(t.dept_ids || 'ALL')}</span></td>
      <td>${t.training_date ? formatDate(t.training_date) : 'TBA'} ${t.training_time ? '• ' + esc(t.training_time) : ''}</td>
      <td>${esc(t.training_type || 'Offline')}</td>
      <td>${t.guide_url ? `<a href="${esc(t.guide_url)}" target="_blank" style="color: var(--lake); font-size: 12px;">Open PDF ↗</a>` : '<span style="color: var(--ink-muted); font-size: 11px;">None</span>'}</td>
      <td style="text-align: right;">
        <button class="btn-icon-action" onclick="openEditTrainingModal(${t.id})">Edit</button>
        <button class="btn-icon-action delete" onclick="deleteTraining(${t.id})">Delete</button>
      </td>
    </tr>
  `).join('');
}
...
// ── 2. REPORTS BOARD ENGINE ───────────────────────────────────
let reportFilterDept = 'all';

function filterReportsByDept(deptId, btn) {
  reportFilterDept = deptId;
  const pills = document.querySelectorAll('#reportsDeptFilterPills .pill');
  pills.forEach(p => p.classList.remove('active'));
  if (btn) btn.classList.add('active');
  renderReports();
}
...
function renderReports() {
  ...
  if (reportFilterDept !== 'all') {
    filtered = filtered.filter(r => String(r.department_id) === String(reportFilterDept));
  }

  if (filtered.length === 0) {
    tbody.innerHTML = `<tr><td colspan="8" style="text-align: center; color: var(--ink-muted); padding: 28px;">No report tasks required for this filter.</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = filtered.map(r => {
    ...
    if (r.status === 'submitted') {
      statusBadge = `<span class="badge badge-green" style="background:#DCFCE7;color:#166534;">✅ Submitted</span>`;
    } else if (r.status === 'overdue') {
      statusBadge = `<span class="badge badge-red" style="background:#FEE2E2;color:#991B1B;">⚠️ Overdue</span>`;
    }

    const actionBtn = r.status === 'submitted'
      ? `<button class="btn-icon-action" onclick="toggleReportStatus(${r.id}, 'pending')" title="Mark as pending again">↩ Reopen</button>`
      : `<button class="btn-secondary" style="font-size: 11.5px; padding: 4px 10px;" onclick="toggleReportStatus(${r.id}, 'submitted')">✓ Mark Submitted</button>`;

    return `
      <tr>
        <td>${statusBadge}</td>
        <td><b>${esc(r.report_name)}</b></td>
        <td><span class="badge badge-lake">${esc(r.department_name || 'Department ' + r.department_id)}</span></td>
        <td><span class="badge" style="text-transform: capitalize;">${esc(r.frequency || 'Monthly')}</span></td>
        <td><b>${r.due_date ? formatDate(r.due_date) : 'Custom'}</b></td>
        <td>${esc(r.added_by || 'Admin')}</td>
        <td><span style="font-size: 12px; color: var(--ink-muted);">${esc(r.custom_note || '—')}</span></td>
...
async function toggleReportStatus(id, newStatus) {
  try {
    ...
    if (json.success) {
      ...
      showToast(json.error || 'Failed to update report status', 'error');
    }
  } catch (err) {
    showToast('Network error while updating report', 'error');
  }
}

function openAddReportModal() {
  populateDropdowns();
  document.getElementById('reportModalTitle').textContent = 'Assign Department Report Task';
  document.getElementById('reportModalEditId').value = '';
  document.getElementById('reportModalNameInput').value = '';
  document.getElementById('reportModalFreqSelect').value = 'monthly';
  document.getElementById('reportModalDateInput').value = '';
  document.getElementById('reportModalNoteInput').value = '';
  openModal('reportModal');
}

function openEditReportModal(id) {
  const r = (DATA.reports || []).find(item => item.id === id);
  if (!r) return;
  populateDropdowns();
  document.getElementById('reportModalTitle').textContent = 'Edit Department Report Task';
  document.getElementById('reportModalEditId').value = r.id;
  document.getElementById('reportModalNameInput').value = r.report_name || '';
  document.getElementById('reportModalDeptSelect').value = r.department_id || '';
  document.getElementById('reportModalFreqSelect').value = r.frequency || 'monthly';
  document.getElementById('reportModalDateInput').value = r.due_date ? r.due_date.split('T')[0] : '';
  document.getElementById('reportModalNoteInput').value = r.custom_note || '';
  openModal('reportModal');
}

async function saveReportRecord() {
  const editId = document.getElementById('reportModalEditId').value;
  const report_name = document.getElementById('reportModalNameInput').value.trim();
  const department_id = document.getElementById('reportModalDeptSelect').value;
  const frequency = document.getElementById('reportModalFreqSelect').value;
  const due_date = document.getElementById('reportModalDateInput').value || null;
  const custom_note = document.getElementById('reportModalNoteInput').value.trim();

  if (!report_name || !department_id) {
    showToast('Please enter report name and select department', 'error');
    return;
  }
  ...
  try {
    ...
    if (json.success) {
      ...
      showToast(editId ? 'Report updated!' : 'Department report task assigned!', 'success');
      if (json.report) {
        const dept = (DATA.departments || []).find(d => d.id === parseInt(department_id));
        const enriched = { ...json.report, department_name: dept?.name };
        if (editId) {
          const idx = (DATA.reports || []).findIndex(r => r.id === parseInt(editId));
          if (idx !== -1) DATA.reports[idx] = enriched;
        } else {
      ...
      showToast(json.error || 'Failed to save report', 'error');
    }
  } catch (err) {
...
async function deleteReport(id) {
  if (!confirm('Are you sure you want to delete this report task?')) return;

  const previousReports = [...(DATA.reports || [])];
  DATA.reports = (DATA.reports || []).filter(r => r.id !== id);
  renderReports();
  renderAdminReportsTable();
  showToast('Report requirement removed', 'success');
...
function renderAdminReportsTable() {
  ...
  if (list.length === 0) {
    tbody.innerHTML = `<tr><td colspan="8" style="text-align: center; color: var(--ink-muted); padding: 24px;">No report tasks configured. Click "+ Add Report Task" to assign one.</td></tr>`;
    return;
  }
  tbody.innerHTML = list.map(r => `
    <tr>
      <td><span class="badge ${r.status === 'submitted' ? 'badge-green' : 'badge-gold'}">${esc(r.status || 'pending')}</span></td>
      <td><b>${esc(r.report_name)}</b></td>
      <td><span class="badge badge-lake">${esc(r.department_name || 'Dept ' + r.department_id)}</span></td>
      <td><span class="badge">${esc(r.frequency || 'Monthly')}</span></td>
      <td><b>${r.due_date ? formatDate(r.due_date) : 'Custom'}</b></td>
      <td>${esc(r.added_by || 'Admin')}</td>
      <td><span style="font-size: 12px; color: var(--ink-muted);">${esc(r.custom_note || '—')}</span></td>
      <td style="text-align: right;">
        <button class="btn-icon-action" onclick="openEditReportModal(${r.id})">Edit</button>
        <button class="btn-icon-action" onclick="toggleReportStatus(${r.id}, '${r.status === 'submitted' ? 'pending' : 'submitted'}')">${r.status === 'submitted' ? 'Reopen' : 'Mark Done'}</button>
        <button class="btn-icon-action delete" onclick="deleteReport(${r.id})">Delete</button>
      </td>
    </tr>
  `).join('');
}
...
// ── 3. COMPANY MEETINGS & SCHEDULES ENGINE ────────────────────
let meetingFilterType = 'all';

function filterMeetings(type, btn) {
  meetingFilterType = type;
  const pills = document.querySelectorAll('#meetingTypePills .pill');
  pills.forEach(p => p.classList.remove('active'));
  if (btn) btn.classList.add('active');
  renderMeetings();
}
...
function renderMeetings() {
  ...
  if (meetingFilterType !== 'all') {
    filtered = filtered.filter(m => m.meeting_type === meetingFilterType);
  }

  if (filtered.length === 0) {
    grid.innerHTML = `<div class="empty-state" style="grid-column: 1/-1;"><p>No scheduled meetings matching '${meetingFilterType}'.</p></div>`;
    return;
  }
  ...
  grid.innerHTML = filtered.map(m => {
    const col = typeColors[m.meeting_type] || '#2A6FA8';
    let audience = '🏢 Company-Wide (All)';
    if (m.dept_ids && m.dept_ids !== 'ALL') {
      const ids = m.dept_ids.split(',').map(n => parseInt(n));
      const depts = DATA.departments.filter(d => ids.includes(d.id));
      audience = depts.map(d => `${d.icon} ${d.name}`).join(', ') || m.dept_ids;
    }

    return `
      <div class="card" style="border-left: 5px solid ${col}; display: flex; flex-direction: column; justify-content: space-between;">
        <div>
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
            <span class="badge" style="background: ${col}15; color: ${col}; font-weight: 700;">${esc(m.meeting_type)} Meeting</span>
            <span class="badge" style="background: #F1F5F9; color: var(--ink-muted);">${esc(m.status || 'scheduled')}</span>
          </div>
          <h4 style="font-size: 17px; margin-bottom: 8px; color: var(--ink);">${esc(m.title)}</h4>
          <div style="font-size: 12.5px; color: var(--ink-muted); display: flex; flex-direction: column; gap: 4px; margin-bottom: 12px;">
            <div>📅 <b>When:</b> ${m.meeting_date ? formatDate(m.meeting_date) : 'TBA'} ${m.meeting_time ? 'at ' + esc(m.meeting_time) : ''}</div>
            <div>📍 <b>Location:</b> ${esc(m.location || 'Online / Meeting Room')}</div>
            <div>👤 <b>Organizer:</b> ${esc(m.organizer || 'Admin')}</div>
            <div>👥 <b>Audience:</b> ${esc(audience)}</div>
          </div>
          ${m.agenda ? `<div style="background: var(--snow); padding: 10px 12px; border-radius: var(--radius-sm); font-size: 12px; color: var(--ink); line-height: 1.4;"><b>Agenda:</b> ${esc(m.agenda)}</div>` : ''}
        </div>
      </div>
    `;
  }).join('');
...
function openAddMeetingModal() {
  populateDropdowns();
  document.getElementById('meetingModalTitle').textContent = 'Schedule Company Meeting';
  document.getElementById('meetingModalEditId').value = '';
  document.getElementById('meetingModalTitleInput').value = '';
  document.getElementById('meetingModalTypeSelect').value = 'General';
  document.getElementById('meetingModalDeptSelect').value = 'ALL';
  document.getElementById('meetingModalDateInput').value = '';
  document.getElementById('meetingModalTimeInput').value = '';
  document.getElementById('meetingModalOrganizerInput').value = currentUser.name || 'Primary Administrator';
  document.getElementById('meetingModalLocationInput').value = 'Main Conference Room / Google Meet';
  document.getElementById('meetingModalAgendaInput').value = '';
  openModal('meetingModal');
}

function openEditMeetingModal(id) {
  const m = (DATA.meetings || []).find(item => item.id === id);
  if (!m) return;
  populateDropdowns();
  document.getElementById('meetingModalTitle').textContent = 'Edit Company Meeting';
  document.getElementById('meetingModalEditId').value = m.id;
  document.getElementById('meetingModalTitleInput').value = m.title || '';
  document.getElementById('meetingModalTypeSelect').value = m.meeting_type || 'General';
  document.getElementById('meetingModalDeptSelect').value = m.dept_ids || 'ALL';
  document.getElementById('meetingModalDateInput').value = m.meeting_date ? m.meeting_date.split('T')[0] : '';
  document.getElementById('meetingModalTimeInput').value = m.meeting_time || '';
  document.getElementById('meetingModalOrganizerInput').value = m.organizer || '';
  document.getElementById('meetingModalLocationInput').value = m.location || '';
  document.getElementById('meetingModalAgendaInput').value = m.agenda || '';
  openModal('meetingModal');
}

async function saveMeetingRecord() {
  const editId = document.getElementById('meetingModalEditId').value;
  const title = document.getElementById('meetingModalTitleInput').value.trim();
  const meeting_type = document.getElementById('meetingModalTypeSelect').value;
  const dept_ids = document.getElementById('meetingModalDeptSelect').value;
  const meeting_date = document.getElementById('meetingModalDateInput').value || null;
  const meeting_time = document.getElementById('meetingModalTimeInput').value.trim();
  const organizer = document.getElementById('meetingModalOrganizerInput').value.trim();
  const location = document.getElementById('meetingModalLocationInput').value.trim();
  const agenda = document.getElementById('meetingModalAgendaInput').value.trim();
  ...
  const endpoint = editId ? `/api/meetings/${editId}` : '/api/meetings';
  const method = editId ? 'PUT' : 'POST';
  ...
  try {
    const res = await fetch(endpoint, {
      method,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title, meeting_type, dept_ids, meeting_date, meeting_time, organizer, location, agenda })
    });
    const json = await res.json();
    if (json.success) {
      closeModal('meetingModal');
      showToast(editId ? 'Meeting updated!' : 'Meeting scheduled!', 'success');
      if (json.meeting) {
        if (editId) {
          const idx = (DATA.meetings || []).findIndex(m => m.id === parseInt(editId));
          if (idx !== -1) DATA.meetings[idx] = json.meeting;
        } else {
          if (!DATA.meetings) DATA.meetings = [];
          DATA.meetings.unshift(json.meeting);
        }
        renderMeetings();
        renderAdminMeetingsTable();
      }
      ...
      showToast(json.error || 'Failed to save meeting', 'error');
    }
  } catch (err) {
...
async function deleteMeeting(id) {
  if (!confirm('Are you sure you want to cancel and remove this meeting?')) return;

  const previousMeetings = [...(DATA.meetings || [])];
  ...
  try {
    ...
    if (!json.success) {
      ...
      renderMeetings();
      renderAdminMeetingsTable();
      showToast(json.error || 'Failed to delete meeting', 'error');
    }
  } catch (err) {
    DATA.meetings = previousMeetings;
    renderMeetings();
...
function renderAdminMeetingsTable() {
  const tbody = document.getElementById('adminMeetingsTableBody');
  if (!tbody) return;
  const list = DATA.meetings || [];
  if (list.length === 0) {
    tbody.innerHTML = `<tr><td colspan="8" style="text-align: center; color: var(--ink-muted); padding: 24px;">No meetings scheduled. Click "+ Schedule Meeting" to add.</td></tr>`;
    return;
  }
  tbody.innerHTML = list.map(m => `
    <tr>
      <td><span class="badge badge-lake">${esc(m.meeting_type)}</span></td>
      <td><b>${esc(m.title)}</b></td>
      <td>${esc(m.dept_ids || 'ALL')}</td>
      <td>${m.meeting_date ? formatDate(m.meeting_date) : 'TBA'} ${m.meeting_time ? '• ' + esc(m.meeting_time) : ''}</td>
      <td>${esc(m.organizer || 'Admin')}</td>
      <td>${esc(m.location || '—')}</td>
      <td><span style="font-size: 12px; color: var(--ink-muted);">${esc(m.agenda || '—')}</span></td>
      <td style="text-align: right;">
        <button class="btn-icon-action" onclick="openEditMeetingModal(${m.id})">Edit</button>
        <button class="btn-icon-action delete" onclick="deleteMeeting(${m.id})">Delete</button>
      </td>
    </tr>
  `).join('');
}
...
// ── 4. ADMIN RESPONSIBILITY PANEL ENGINE ──────────────────────
function renderAdminRoles() {
  ...
  if (roles.length === 0) {
    grid.innerHTML = `<div class="empty-state" style="grid-column: 1/-1;"><p>No administrator roles assigned yet. Admins can configure roles in Admin Area.</p></div>`;
    return;
  }
  ...
  grid.innerHTML = roles.map(r => {
    ...
    for (const [key, ic] of Object.entries(icons)) {
      if (r.responsibility_area && r.responsibility_area.includes(key)) {
        icon = ic;
        break;
      }
    }
    ...
    return `
      <div class="card" style="border-top: 4px solid var(--lake); display: flex; flex-direction: column; justify-content: space-between; position: relative;">
        <div>
          <div style="display: flex; align-items: flex-start; justify-content: space-between; gap: 10px; margin-bottom: 12px;">
            <div style="display: flex; align-items: center; gap: 10px;">
              <div style="width: 44px; height: 44px; border-radius: 12px; background: #EFF6FF; border: 1.5px solid #BFDBFE; display: flex; align-items: center; justify-content: center; font-size: 22px; flex-shrink: 0;">${icon}</div>
              <div>
                <div style="font-size: 11.5px; font-weight: 700; color: var(--lake); text-transform: uppercase; letter-spacing: 0.04em;">${esc(r.responsibility_area)}</div>
                <h4 style="font-size: 18px; margin: 0; color: var(--ink); line-height: 1.3;">${esc(r.admin_name)}</h4>
              </div>
            </div>
            <div style="display: flex; gap: 5px; flex-shrink: 0;">
              <button class="btn-icon-action" onclick="openEditAdminRoleModal(${r.id})" title="Edit responsibility" style="padding: 4px 8px; font-size: 12px;">✏️ Edit</button>
              <button class="btn-icon-action delete" onclick="deleteAdminRole(${r.id})" title="Delete role" style="padding: 4px 8px; font-size: 12px;">🗑️ Delete</button>
            </div>
          </div>
          <p style="font-size: 13.5px; color: var(--ink-light); line-height: 1.5; margin-bottom: 14px;">
            ${esc(r.description || 'Oversees governance, execution, and team inquiries in this domain.')}
          </p>
        </div>
        <div style="border-top: 1px dashed var(--border); padding-top: 10px; display: flex; justify-content: space-between; align-items: center;">
          <span style="font-size: 11.5px; color: var(--ink-muted);">Point of Contact for Staff</span>
          <button class="btn-secondary" style="font-size: 11.5px; padding: 4px 10px;" onclick="switchTab('chat'); insertChatTag('@' + '${esc(r.admin_name)}'.split(' ')[0])">
            💬 Tag in Chat
          </button>
        </div>
      </div>
...
function openAddAdminRoleModal() {
  document.getElementById('adminRoleModalTitle').textContent = 'Assign Admin Responsibility';
  document.getElementById('adminRoleModalEditId').value = '';
  document.getElementById('adminRoleModalNameInput').value = currentUser.name || '';
  document.getElementById('adminRoleModalAreaSelect').value = 'Announcements & Company Updates';
  document.getElementById('adminRoleModalDescInput').value = '';
  const saveBtn = document.getElementById('adminRoleModalSaveBtn');
  if (saveBtn) saveBtn.textContent = 'Save Responsibility';
  openModal('adminRoleModal');
}

function openEditAdminRoleModal(id) {
  const r = (DATA.adminRoles || []).find(item => item.id === id);
  if (!r) return;
  document.getElementById('adminRoleModalTitle').textContent = 'Edit Admin Responsibility';
  document.getElementById('adminRoleModalEditId').value = r.id;
  document.getElementById('adminRoleModalNameInput').value = r.admin_name || '';

  const selectEl = document.getElementById('adminRoleModalAreaSelect');
  if (selectEl) {
    let foundOption = false;
    for (let opt of selectEl.options) {
      if (opt.value === r.responsibility_area) {
        foundOption = true;
        break;
      }
    }
    if (!foundOption && r.responsibility_area) {
      const opt = document.createElement('option');
      opt.value = r.responsibility_area;
      opt.textContent = r.responsibility_area;
      selectEl.appendChild(opt);
    }
    selectEl.value = r.responsibility_area || 'General Administration';
  }

  document.getElementById('adminRoleModalDescInput').value = r.description || '';
  const saveBtn = document.getElementById('adminRoleModalSaveBtn');
  if (saveBtn) saveBtn.textContent = 'Update Responsibility';
  openModal('adminRoleModal');
}

async function saveAdminRoleRecord() {
  const editId = document.getElementById('adminRoleModalEditId').value;
  const admin_name = document.getElementById('adminRoleModalNameInput').value.trim();
  const responsibility_area = document.getElementById('adminRoleModalAreaSelect').value;
  const description = document.getElementById('adminRoleModalDescInput').value.trim();
  ...
  const endpoint = editId ? `/api/admin-roles/${editId}` : '/api/admin-roles';
  const method = editId ? 'PUT' : 'POST';
  ...
  try {
    const res = await fetch(endpoint, {
      method,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ admin_name, responsibility_area, description })
    });
    const json = await res.json();
    if (json.success) {
      closeModal('adminRoleModal');
      showToast(editId ? 'Admin role updated!' : 'Admin responsibility assigned!', 'success');
      if (json.adminRole) {
        if (editId) {
          const idx = (DATA.adminRoles || []).findIndex(r => r.id === parseInt(editId));
          if (idx !== -1) DATA.adminRoles[idx] = json.adminRole;
        } else {
          if (!DATA.adminRoles) DATA.adminRoles = [];
          DATA.adminRoles.unshift(json.adminRole);
        }
        renderAdminRoles();
        renderAdminRolesTable();
      }
      ...
      showToast(json.error || 'Failed to save admin role', 'error');
    }
  } catch (err) {
...
async function deleteAdminRole(id) {
  if (!confirm('Are you sure you want to remove this responsibility assignment?')) return;

  const previousRoles = [...(DATA.adminRoles || [])];
  DATA.adminRoles = (DATA.adminRoles || []).filter(r => r.id !== id);
  renderAdminRoles();
  renderAdminRolesTable();
...
function renderAdminRolesTable() {
  ...
  if (!tbody) return;
  const list = DATA.adminRoles || [];
  if (list.length === 0) {
    tbody.innerHTML = `<tr><td colspan="4" style="text-align: center; color: var(--ink-muted); padding: 24px;">No admin responsibility roles defined. Click "+ Assign Responsibility" to set up.</td></tr>`;
    return;
  }
  tbody.innerHTML = list.map(r => `
    <tr>
      <td><b>${esc(r.admin_name)}</b></td>
      <td><span class="badge badge-lake">${esc(r.responsibility_area)}</span></td>
      <td><span style="font-size: 13px; color: var(--ink-light);">${esc(r.description || '—')}</span></td>
      <td style="text-align: right;">
        <button class="btn-icon-action" onclick="openEditAdminRoleModal(${r.id})">✏️ Edit</button>
        <button class="btn-icon-action delete" onclick="deleteAdminRole(${r.id})">🗑️ Delete</button>
      </td>
    </tr>
  `).join('');
}
